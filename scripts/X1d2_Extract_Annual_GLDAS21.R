...
GLDASJulAug <- ee$ImageCollection('users/crollinson/GLDAS_Annual_JulAug')
ee_print(GLDASJulAug)
# Map$addLayer(GLDASJulAug$first()$select('Tair_f_inst_mean'), vizTempK, "Jul/Aug Temperature")
...
GLDASJanFeb <- ee$ImageCollection('users/crollinson/GLDAS_Annual_JanFeb')
# Map$addLayer(GLDASJanFeb$first()$select('Tair_f_inst_mean'), vizTempK, "Jul/Aug Temperature")
...
extractGLDAS <- function(CitySP, CityNames, GLDAS, GoogleFolderSave, overwrite=F, ...){
  ...
  for(i in 1:length(CityNames)){
    ...
    # ee_print(gldasCity)
    # ee_print(gldasCity$first())
    # Map$addLayer(gldasCity$first()$select("Tair_f_inst_mean"), vizTempK, 'Temperature')
    # Map$addLayer(gldasCity$first()$select('Evap_tavg_mean'), vizPrecip, "Jul/Aug ET")
    # Map$addLayer(gldasCity$first()$select('Rainf_f_tavg_mean'), vizPrecip, "Jul/Aug PR")
    

    # # Code from NDVI work tor educe to a single value --> will need to be inside a map function()
    tableYr <- ee$FeatureCollection(gldasCity$map(function(img){
      RedMn =img$reduceRegion(reducer= ee$Reducer$mean(), geometry=cityNow$geometry(),
                              scale=27829.87, # hard-coded, but it's what has to happen to work
                              maxPixels=1e4)
      # RedMn$getInfo()  
...
if(!overwrite){
  ...
  gldas.done <- dir(file.path(path.google, GoogleFolderSave), "_GLDAS21_annualMeans")
