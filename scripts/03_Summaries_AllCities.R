...
rmse.histo.biome <- ggplot(data=cityAll.stats[!is.na(cityAll.stats$biome),])+
  geom_histogram(aes(x=LSTmodelFinal.RMSE, fill=biomeName)) +
  scale_fill_manual(name="biome", values=biome.pall.all) +
  theme_bw() +
  theme(legend.position="right",
        legend.title=element_text(color="black", face="bold"),
...
rmseMean.histo.biome <- ggplot(data=cityAll.stats[!is.na(cityAll.stats$biome),])+
  geom_histogram(aes(x=LSTmodelFinal.RMSE/LST.mean, fill=biomeName)) +
  scale_fill_manual(name="biome", values=biome.pall.all) +
  theme_bw() +
  theme(legend.position="right",
        legend.title=element_text(color="black", face="bold"),
...
rmseSD.histo.biome <- ggplot(data=cityAll.stats[!is.na(cityAll.stats$biome),])+
  geom_histogram(aes(x=LSTmodelFinal.RMSE/LST.sd, fill=biomeName)) +
  scale_fill_manual(name="biome", values=biome.pall.all) +
  theme_bw() +
  theme(legend.position="right",
        legend.title=element_text(color="black", face="bold"),
...
png(file.path(path.figs, "LSTmodel_PerformanceSummaries.png"), height=8, width=8, units="in", res=320)
cowplot::plot_grid(r2.histo.biome+guides(fill="none"), 
                   rmse.histo.biome+guides(fill="none"),
                   rmseMean.histo.biome+guides(fill="none"), 
                   rmseSD.histo.biome+guides(fill="none"), ncol=2)
dev.off()
...
# ##########################
splineTree <- readRDS(file.path(path.google, "LSTModel_Spline_PartialEffects_CoverTree.rds"))
splineVeg <- readRDS(file.path(path.google, "LSTModel_Spline_PartialEffects_CoverVeg.rds"))
# summary(splineTree)

dfSplineTree <- data.table::rbindlist(splineTree)
dfSplineTree <- merge(dfSplineTree, cityAll.stats[,c("ISOURBID", "biome", "biomeName")], all.x=T, all.y=F)
summary(dfSplineTree)


ggplot(data=dfSplineTree, aes(x=cover.tree, y=LST.pred)) +
  facet_wrap(~biomeName) +
  geom_line(aes(group=ISOURBID), linewidth=0.1, alpha=0.5) + 
  geom_smooth(aes(color=biomeName), method="lm") +
  scale_color_manual(values=biome.pall.all[]) +
  theme_bw()

ggplot(data=dfSplineTree[!dfSplineTree$biomeName %in% c("Tundra", "Flooded Grassland/Savanna", "Tropical Conifer Forest", "Mangroves")], aes(x=cover.tree, y=LST.pred)) +
  # facet_wrap(~biomeName) +
  # geom_line(aes(group=ISOURBID), linewidth=0.1, alpha=0.5) + 
  geom_smooth(aes(color=biomeName), method="lm") +
  scale_color_manual(values=biome.pall.all[]) +
...
ggplot(data=dfSplineTree, aes(x=cover.tree, y=effect.tree)) +
  facet_wrap(~biomeName) +
  geom_line(aes(group=ISOURBID), linewidth=0.1, alpha=0.5) + 
  geom_smooth(aes(color=biomeName), method="lm") +
  scale_color_manual(values=biome.pall.all[]) +
...
cityAll.stats$effect.tree.degC <- cityAll.stats$tree.mean*cityAll.stats$LSTslope.tree
cityAll.stats$effect.veg.degC <- cityAll.stats$veg.mean*cityAll.stats$LSTslope.veg
summary(cityAll.stats)
...
# 
# # ##########################################
# # Looking at some of the ET stuff ----
# # ##########################################
# summary(cityAll.stats)
# ggplot(data=cityAll.stats) +
