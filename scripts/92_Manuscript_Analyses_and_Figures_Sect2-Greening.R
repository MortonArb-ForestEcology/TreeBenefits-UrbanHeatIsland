...
StatsCombined$greening.treeAdd <- StatsCombined$tree.mean.TreeTargetBottomUp - StatsCombined$value.tree.core
StatsCombined$greening.coolingAdd <- StatsCombined$greening.treeAdd*StatsCombined$LSTmodel.tree.slope
StatsCombined$greening.treeTotal <- StatsCombined$tree.mean.TreeTargetBottomUp # Yeah, just renaming, but it will help below
StatsCombined$greening.coolingTotal <- StatsCombined$tree.mean.TreeTargetBottomUp*StatsCombined$LSTmodel.tree.slope
StatsCombined$greening.remainUHI <- StatsCombined$value.LST.diff + StatsCombined$greening.coolingTotal
summary(StatsCombined)

# Water Ratios
StatsCombined$ETcurrent.Precip <- StatsCombined$modET.Base/StatsCombined$Precip.GLDAS
StatsCombined$ETgreenTarget.Precip <- StatsCombined$modET.TreeTargetBottomUp/StatsCombined$Precip.GLDAS
summary(StatsCombined)
...
summary(StatsCombined[,c("value.tree.core", "TreeCoverUHINeed", "greening.treeAdd")]) 

summary((StatsCombined$greening.treeAdd+StatsCombined$value.tree.core)/StatsCombined$value.tree.core) # This would be a ratio of current canopy 
...
summary(StatsCombined$greening.treeAdd/StatsCombined$TreeCoverUHINeed) # Note: this is the same as doing it with temperature because the linear relationship & how our goal is set up
...
TreeCoverTarget <- ggplot(data=StatsCombined[,], aes(x=biomeCode, y=TreeCoverTargetUHI, fill="Biome Target", color="Biome Target")) +  
  ...
  geom_violin(aes(x=biomeCode, y=tree.mean.TreeTargetBottomUp, fill="50% Greening", color="50% Greening"), scale="width", alpha=0.5) +
  annotate("text", x=10.5, y=33, label="30%", hjust=1, vjust=0) +
  # geom_point(stat="summary", fun="median", size=5) +
  scale_fill_manual(name="Tree Cover", values=c("Current"="#005a32", "Biome Target"=grad.tree[4], "50% Greening"=grad.tree[6])) +
  scale_color_manual(name="Tree Cover", values=c("Current"="#005a32", "Biome Target"=grad.tree[4], "50% Greening"=grad.tree[6])) +
...
effectsUHI$GreeningTarget <- stack(StatsCombined[,c("greening.coolingTotal", "greening.remainUHI")])[,1]
...
effectsUHI2 <- stack(effectsUHI[,c("Current", "GreeningTarget")])
...
effectsET <- stack(StatsCombined[,c("modET.Base", "modET.TreeTargetBottomUp")])
names(effectsET) <- c("ET", "Scenario")
effectsET$ETratio <- stack(StatsCombined[,c("ETcurrent.Precip", "ETgreenTarget.Precip")])[,1]
effectsET$Scenario <- as.character(effectsET$Scenario)
effectsET$Scenario[grepl("Base", effectsET$Scenario)] <- "Current"
effectsET$Scenario[grepl("Target", effectsET$Scenario)] <- "GreeningTarget"
